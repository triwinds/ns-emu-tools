...
```powershell
cargo fmt --manifest-path src-tauri/crates/streamline-layer-probe/Cargo.toml
cargo check --locked --manifest-path src-tauri/crates/streamline-layer-probe/Cargo.toml
cargo check --locked --manifest-path src-tauri/crates/streamline-layer-probe/Cargo.toml --target x86_64-pc-windows-msvc
cargo test --locked --manifest-path src-tauri/crates/streamline-layer-probe/Cargo.toml
cargo build --locked --manifest-path src-tauri/crates/streamline-layer-probe/Cargo.toml

& D:\py\ns-emu-tools\src-tauri\crates\streamline-layer-probe\target\debug\streamline-layer-probe.exe `
  --layer D:\py\ns-emu-tools\src-tauri\crates\streamline-layer-probe\target\debug\streamline_probe_layer.dll `
...
缺少时，在仓库根目录克隆到以上 target 目录；已有目录不要覆盖：
    ...
    cargo build --locked --manifest-path src-tauri/crates/streamline-layer-probe/Cargo.toml --features sdk-bridge
